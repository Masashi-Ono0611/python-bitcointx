...
def read_inputs() -> tuple[str, str, int, int, int, str]:
    print("=== python-bitcointx: P2WPKH PSBT SIGHASH_SINGLE|ANYONECANPAY 1-in-1-out demo ===\n")
...
def build_psbt_p2wpkh_sighash_single_anyonecanpay_1in1out(
    wif: str,
    prev_txid_hex: str,
    vout: int,
    utxo_value: int,
    ...
    print("\n[Debug] SIGHASH comparison (1-in-1-out):")
...
def main() -> None:
    try:
        ...
        psbt = build_psbt_p2wpkh_sighash_single_anyonecanpay_1in1out(
            wif,
            prev_txid_hex,
            vout,
            utxo_value,
    ...
    print("\n=== Signed transaction (PSBT, SIGHASH_SINGLE|ANYONECANPAY, 1-in-1-out) ===")
    print(f"TxID      : {txid}")
